...
import { opportunityData } from "@/lib/mockData";
...
export default function Home() {
  return (
    <div className="min-h-screen bg-background text-foreground font-sans selection:bg-primary/10">
      {/* 1. HEADER */}
      <header className="border-b border-border/60 bg-white/50 backdrop-blur-sm sticky top-0 z-50">
        <div className="container mx-auto px-6 py-6 max-w-7xl">
          <h1 className="text-3xl md:text-4xl font-serif font-medium text-primary mb-2">
            How AI Is Changing Work — Tasks, Skills, Opportunities
          </h1>
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <p className="text-lg text-muted-foreground font-light">
              A research-based view on AI, jobs, and labor market change
            </p>
            <div className="text-xs text-muted-foreground bg-secondary/50 px-3 py-1 rounded-full border border-border/50">
              This platform explains patterns and trends. It does not predict individual outcomes.
            </div>
          </div>
        </div>
      </header>

      <main className="container mx-auto px-6 py-8 max-w-7xl space-y-12">
        
        {/* 2. TOP SUMMARY STRIP */}
        <section className="bg-primary/5 border border-primary/10 rounded-xl p-6 md:p-8 relative overflow-hidden">
           ...
           <div className="grid md:grid-cols-3 gap-6 text-sm md:text-base">
             <div className="space-y-1">
               ...
               <p className="font-medium text-foreground">In the selected context, AI is mainly <span className="text-primary font-semibold border-b border-primary/30">augmenting tasks</span> rather than replacing jobs.</p>
             </div>
             <div className="space-y-1">
               <span className="text-xs font-semibold text-primary/60 uppercase tracking-wider">Skill Pressure</span>
               <p className="font-medium text-foreground">Skill requirements are changing <span className="text-amber-600/80 font-semibold border-b border-amber-600/20">moderately</span> across most sectors.</p>
             </div>
             <div className="space-y-1">
                <span className="text-xs font-semibold text-primary/60 uppercase tracking-wider">Demand Signal</span>
                <p className="font-medium text-foreground">Labor demand remains <span className="text-emerald-600/80 font-semibold border-b border-emerald-600/20">strong</span> in several adjacent human-centric roles.</p>
             </div>
           </div>
        </section>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          {/* SECTION A — TASK TRANSFORMATION */}
          <section className="space-y-4">
             <TaskTransformationChart />
          </section>

          {/* SECTION B — SKILL CHANGE DYNAMICS */}
          <section className="space-y-4">
             <SkillDynamicsChart />
          </section>
            ...
            <div>
              <Card className="h-full border-none shadow-sm bg-white/50 backdrop-blur-sm">
                <CardHeader>
                  ...
                  <CardDescription>
                    Structural opportunities, not guarantees.
                  </CardDescription>
                </CardHeader>
                <CardContent>
          ...
          <div className="grid md:grid-cols-3 gap-8 mb-8">
            <div>
              ...
              <p className="text-sm text-muted-foreground leading-relaxed">
                Aggregated from public workforce datasets, academic research on automation exposure, and labor market trend analysis. 
                (Placeholder for specific citations).
              </p>
            </div>
            <div>
